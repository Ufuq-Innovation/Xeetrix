...
export default function UnifiedOrderPage() {
  ...
  const filteredOrders = useMemo(() => {
    return orders.filter(order => {
      ...
      return (order.customerName || '').toLowerCase().includes(searchStr) || 
    ...
    <div className="space-y-8 p-4 md:p-6 max-w-[1600px] mx-auto text-slate-200">
        ...
        <div className="xl:col-span-3 space-y-6">
          <div className="bg-[#11161D] rounded-[2.5rem] border border-white/5 p-6 md:p-8 shadow-2xl">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <input className="w-full px-5 py-4 bg-[#1a2230] border border-white/10 rounded-2xl text-sm font-bold outline-none focus:border-blue-500" placeholder="Customer Phone" value={customerInfo.phone} onChange={e => setCustomerInfo({...customerInfo, phone: e.target.value})} />
            ...
            <button onClick={() => createOrderMutation.mutate({ 
              ...
              })} className="w-full mt-8 py-6 rounded-3xl bg-blue-600 font-black uppercase shadow-2xl active:scale-95 transition-all">
              Confirm Transaction
            </button>
          </div>
        </div>
      ...
      <div className="bg-[#11161D] rounded-[2.5rem] border border-white/5 overflow-hidden shadow-2xl mt-12">
        <div className="p-8 border-b border-white/5 flex justify-between items-center">
          <h2 className="text-lg font-black uppercase italic tracking-widest">Transaction History</h2>
          <input className="px-5 py-3 bg-black/20 border border-white/10 rounded-2xl text-xs outline-none" placeholder="Search orders..." value={filters.search} onChange={e => setFilters({...filters, search: e.target.value})} />
        </div>
            ...
            <tbody className="divide-y divide-white/5 text-[11px] font-bold">
              {filteredOrders.map((order, i) => (
                <tr key={i} className="hover:bg-white/[0.02] transition-colors">
                  <td className="px-6 py-4"><span className="text-blue-500 font-mono">{order.orderId}</span><br/><span className="text-[9px] text-slate-600 uppercase">{new Date(order.createdAt || Date.now()).toLocaleDateString()}</span></td>
                  <td className="px-6 py-4 uppercase">{order.customerName}</td>
                  <td className="px-6 py-4 text-right font-black text-slate-200">{currency}{(order.totalSell || 0).toLocaleString()}</td>
                  <td className="px-6 py-4 text-center"><button className="p-2 hover:bg-white/10 rounded-lg"><Eye size={16} className="text-slate-500"/></button></td>
                </tr>
              ))}
            </tbody>
          </table>
        ...
        <div className="fixed inset-0 z-[100] flex items-center justify-center p-4 bg-black/90 backdrop-blur-md">
          <div className="bg-white rounded-[2.5rem] w-full max-w-sm overflow-hidden text-slate-900 shadow-2xl">
            <div ref={receiptRef} className="p-8 bg-white">
              ...
              <div className="space-y-1 text-[10px] mb-6 uppercase font-black text-slate-600">
                <div className="flex justify-between"><span>Customer:</span><span>{lastSavedOrder.customerName}</span></div>
              ...
              <div className="space-y-2 mb-6 bg-slate-50 p-6 rounded-3xl border border-slate-100">
                 <div className="flex justify-between text-[11px] font-bold text-red-500"><span>Discount</span><span>-{currency}{(lastSavedOrder.discount || 0).toLocaleString()}</span></div>
                 {lastSavedOrder.transactionType === "online" && <div className="flex justify-between text-[11px] font-bold text-blue-500"><span>Courier</span><span>+{currency}{(lastSavedOrder.courier || 0).toLocaleString()}</span></div>}
                 <div className="border-t border-slate-200 pt-3 mt-3 flex justify-between font-black text-2xl tracking-tighter text-slate-900"><span>Total Paid</span><span>{currency}{(lastSavedOrder.totalSell || 0).toLocaleString()}</span></div>
              </div>
              <p className="text-center text-[8px] font-black text-slate-300 uppercase italic">Invoice Generated Successfully</p>
            </div>

            <div className="p-6 bg-slate-100 grid grid-cols-2 gap-3 border-t border-slate-200">
                <button onClick={() => window.print()} className="bg-slate-900 text-white py-4 rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 hover:bg-black">
                  <Printer size={14}/> PRINT PDF
                </button>
                <button onClick={downloadReceiptImage} className="bg-blue-600 text-white py-4 rounded-2xl font-black uppercase text-[10px] flex items-center justify-center gap-2 hover:bg-blue-700">
