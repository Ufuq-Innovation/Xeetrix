...
import { useTranslation } from 'react-i18next'; // Reactivity ensuring hook
...
export default function Dashboard() {
  // Access language from context to use as a dependency for TanStack Query
  const { lang } = useApp();
  const { t } = useTranslation('common'); // Hook for real-time translation

  /**
   * Fetch dashboard statistics with TanStack Query.
   * Key is linked to 'lang' to ensure data re-fetch if currency formatting changes.
   */
