...
export default function FinancePage() {
  ...
  /** Fetch financial data and statistics */
  const fetchData = async () => {
    try {
      const [resExp, resStats] = await Promise.all([
        fetch('/api/finance'),
        fetch('/api/dashboard') // stats API rename standard consistent with your map
      ]);
    ...
    <div className="space-y-10 p-4 md:p-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-[#11161D] p-8 rounded-[2.5rem] border border-white/5 space-y-6">
          <h3 className="text-xl font-bold text-slate-200">Record New Expense</h3>
          <form onSubmit={handleExpenseSubmit} className="space-y-4">
            <input 
              type="text" placeholder="Description (e.g., Facebook Ads)" required
        ...
        <div className="bg-[#11161D] p-8 rounded-[2.5rem] border border-white/5 flex flex-col justify-center items-center space-y-4 relative overflow-hidden">
          ...
          <div className="text-center space-y-1">
            <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">
              Order Profit: <span className="text-white">৳{Number(stats.totalProfit) + Number(stats.totalExpense || 0)}</span>
            </p>
            <p className="text-[11px] text-red-400 font-medium uppercase tracking-wider">
              Total Expenses: <span>৳{stats.totalExpense || 0}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#11161D] rounded-[2.5rem] border border-white/5 overflow-hidden">
        <div className="p-8 border-b border-white/5 flex justify-between items-center">
          <h3 className="font-bold uppercase tracking-widest text-sm flex items-center gap-2 text-slate-300">
            <ArrowDownCircle size={18} className="text-blue-500" /> Transaction Log
          </h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            ...
            <tbody className="text-white">
              {fetching ? (
                <tr><td colSpan="4" className="p-10 text-center text-slate-500 animate-pulse">Synchronizing records...</td></tr>
              ) : expenses.length === 0 ? (
                <tr><td colSpan="4" className="p-10 text-center text-slate-500">No transactions recorded yet.</td></tr>
              ) : expenses.map((exp) => (
