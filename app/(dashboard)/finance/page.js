...
import { useTranslation } from 'react-i18next'; // Hook for reactive translation updates
...
export default function FinancePage() {
  ...
  /**
   * Fetch both expense records and business stats simultaneously.
   */
  ...
  // Re-fetch data on mount or when language context updates
  useEffect(() => { 
    fetchData(); 
  }, [lang]);

  /**
   * Handle submission of new expense entries.
   */
  ...
  /**
   * Delete specific expense record after confirmation.
   */
    ...
    <div className="space-y-10 p-4 md:p-0">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
        ...
        <div className="bg-[#11161D] p-8 rounded-[2.5rem] border border-white/5 space-y-6">
          <h3 className="text-xl font-bold text-slate-200">{t('new_expense')}</h3>
          <form onSubmit={handleExpenseSubmit} className="space-y-4">
            <input 
              type="text" placeholder={t('expense_desc_placeholder')} required
            ...
            />
            <input 
              type="number" placeholder={t('amount_placeholder')} required
              className="w-full bg-white/5 p-4 rounded-xl border border-white/10 outline-none focus:border-blue-600 text-white"
        ...
        <div className="bg-[#11161D] p-8 rounded-[2.5rem] border border-white/5 flex flex-col justify-center items-center space-y-4 relative overflow-hidden">
          ...
          <p className="text-slate-500 font-bold uppercase text-xs tracking-widest">{t('net_cash_flow')}</p>
          <h2 className={`text-6xl font-black tracking-tighter ${stats.totalProfit >= 0 ? 'text-green-500' : 'text-red-500'}`}>
            ৳ {stats.totalProfit}
          </h2>
          <div className="text-center space-y-1">
            <p className="text-[11px] text-slate-400 font-medium uppercase tracking-wider">
              {t('order_profit')}: <span className="text-white">৳{Number(stats.totalProfit) + Number(stats.totalExpense || 0)}</span>
            </p>
            <p className="text-[11px] text-red-400 font-medium uppercase tracking-wider">
              {t('total_expenses')}: <span>৳{stats.totalExpense || 0}</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div className="bg-[#11161D] rounded-[2.5rem] border border-white/5 overflow-hidden shadow-2xl">
        <div className="p-8 border-b border-white/5 flex justify-between items-center">
          <h3 className="font-bold uppercase tracking-widest text-sm flex items-center gap-2 text-slate-300">
            <ArrowDownCircle size={18} className="text-blue-500" /> {t('transaction_log')}
          </h3>
        </div>
        <div className="overflow-x-auto">
          <table className="w-full text-left">
            <thead className="text-[10px] text-slate-500 font-black uppercase border-b border-white/5">
              <tr>
                <th className="p-6">{t('description')}</th>
                <th className="p-6">{t('category')}</th>
            ...
            <tbody className="text-white">
              {fetching ? (
                <tr><td colSpan="4" className="p-10 text-center text-slate-500 animate-pulse">{t('syncing_records')}</td></tr>
              ) : expenses.length === 0 ? (
                <tr><td colSpan="4" className="p-10 text-center text-slate-500 italic uppercase text-xs">{t('no_transactions')}</td></tr>
              ) : expenses.map((exp) => (
