    ...
    <div className="space-y-10">
      ...
      <form onSubmit={handleAddProduct} className="grid grid-cols-1 md:grid-cols-4 gap-4 bg-[#11161D] p-6 rounded-[2rem] border border-white/5">
        <input 
          ...
          className="bg-white/5 p-4 rounded-xl border border-white/10 text-white outline-none focus:border-blue-600"
            ...
            {fetching ? (
              ...
              products.map((item) => (
                <tr key={item._id} className="border-b border-white/5 hover:bg-white/[0.02]">
                  ...
                  <td className="p-6">
                    {Number(item.stock) > 5 ? 
                      <span className="text-green-500 bg-green-500/10 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest">In Stock</span> : 
                      <span className="text-red-500 bg-red-500/10 px-3 py-1 rounded-full text-[10px] font-bold uppercase tracking-widest">Low Stock</span>
                    }
                  </td>
                </tr>
              ))
